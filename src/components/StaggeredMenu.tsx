...
export interface StaggeredMenuProps {
  ...
  // Custom props for scroll-based integration
  isOpenOnScroll?: boolean;
...
export const StaggeredMenu: React.FC<StaggeredMenuProps> = ({
  ...
  const toggleMenu = useCallback((forceState?: boolean) => {
    const target = forceState !== undefined ? forceState : !openRef.current;
    if (target === openRef.current) return; // Prevent redundant animations
  ...
  useEffect(() => {
    if (!closeOnClickAway || !open || isOpenOnScroll !== undefined) return;
  ...
  }, [closeOnClickAway, open, toggleMenu, isOpenOnScroll]);
  
  // React to external scroll prop
  useEffect(() => {
    if (isOpenOnScroll !== undefined) {
      toggleMenu(isOpenOnScroll);
    }
  }, [isOpenOnScroll, toggleMenu]);

  return (
    <div className={cn(
      "sm-scope overflow-hidden select-none font-sans",
      isFixed ? "fixed inset-0 z-[100] pointer-events-none" : "relative w-full h-full pointer-events-none",
      className
    )}>
      <div 
      ...
      >
        ...
        <div ref={preLayersRef} className={cn(
          "sm-prelayers absolute top-0 bottom-0 pointer-events-none z-[5] w-[100vw]",
        ...
        {/* Header with Logo & Toggle (Hidden when controlled by scroll) */}
        {isOpenOnScroll === undefined && (
        ...
        <aside
          ref={panelRef}
          className={cn(
            "staggered-menu-panel z-10 pointer-events-auto flex flex-col w-[100vw]",
            isFixed ? "absolute top-0 bottom-0 pt-32 pb-12 px-8 sm:px-16 overflow-y-auto" : "relative min-h-screen",
            position === 'left' ? 'left-0' : 'right-0'
          )}
              ...
              <>
                      ...
                      <li key={idx} className="overflow-hidden">
                        ...
                        >
                          {displayItemNumbering && (
                            <span className="text-sm font-medium opacity-40 translate-y-[-0.5rem] text-black">
                ...
                {displaySocials && socialItems.length > 0 && (
                  <div className="mt-auto pt-12">
                    <h3 className="sm-socials-title text-xs font-bold uppercase tracking-widest mb-6 opacity-40 text-white">Socials</h3>
                      ...
                      {socialItems.map((social, i) => (
                        <li key={i}>
                          <a
                            ...
                            className="sm-socials-link text-sm font-medium text-white/80 no-underline hover:text-[var(--sm-accent)] transition-colors py-1 inline-block"
                          >
                            {social.label}
                          </a>
                        </li>
